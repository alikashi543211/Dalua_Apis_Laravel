                ...
                <div class="card shadow">
                    <div class="card-header border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">Edit Advance Schedule</h3>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.schedules.update') }}" method="POST">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="name" required id="name" value="{{ $schedule->name }}">
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                            ...
                            @for ($i = 0; $i < 6; $i++) <div class="row mt-4" id="row_{{ $i }}">
                                <div class="col">
                                    <div class="row">
                                        <div class="col">
                                            <div class="form-group">
                                                <label for="channel_a_start_{{ $i }}">Channel A</label>
                                                <input @if($i==5) readonly @endif data-id="{{ $i }}" data-channel="a" required type="number" min="0" max="100" class="form-control start_value" id="channel_a_start_{{ $i }}" name="value_a[]" value="{{ $schedule->slots[$i]->value_a }}">
                                            </div>
                                        </div>
                                        <div class="col">
                                            <div class="form-group">
                                                <label for="channel_b_start_{{ $i }}">Channel B</label>
                                                <input @if($i==5) readonly @endif data-id="{{ $i }}" data-channel="b" required type="number" min="0" max="100" class="form-control start_value" id="channel_b_start_{{ $i }}" name="value_b[]" value="{{ $schedule->slots[$i]->value_b }}">
                                            </div>
                                        </div>
                                        <div class="col">
                                            <div class="form-group">
                                                <label for="channel_c_start_{{ $i }}">Channel C</label>
                                                <input @if($i==5) readonly @endif data-id="{{ $i }}" data-channel="c" required type="number" min="0" max="100" class="form-control start_value" id="channel_c_start_{{ $i }}" name="value_c[]" value="{{ $schedule->slots[$i]->value_c }}">
                                            </div>
                                        </div>
                                        <div class="col">
                                            <div class="form-group">
                                                <label for="start_time_{{ $i }}">Start Time</label>
                                                {{-- <input required type="text" class="form-control start-time timepicker" data-id="{{ $i }}" id="start_time_{{ $i }}" name="start_time[]" value="{{ date('h:i A', strtotime($schedule->slots[$i]->start_time)) }}"> --}}
                                                <select data-id="{{ $i }}" id="start_time_{{ $i }}" name="start_time[]" required class="form-control">
                                                    <option value="">Select Time</option>
                                                    @foreach(config('time_slots') as $slot)
                                                        <option value="{{ $slot }}" @if(date('h:i A', strtotime($schedule->slots[$i]->start_time)) == $slot) selected @endif>{{ $slot }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col d-flex justify-content-center align-items-end">
                                            <button type="button" class="btn btn-primary form-group modal-button" data-target="#graph-modal" data-id="{{ $i }}" data-start_end="start">View Spectograph</button>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col"></div>
                                        <div class="col text-center">
                                            <label>
                                                <input type="radio" class="radio-type-select" data-id="{{ $i }}" name="type_{{ $i }}" value="{{ TYPE_STEP }}" {{ $schedule->slots[$i]->type == TYPE_STEP ? 'checked' : ''}}>
                                                <span><i class="fas fa-chart-line"></i> STEP</span>
                                            </label>
                                        </div>
                                        <div class="col text-center">
                                            <label>
                                                <input type="radio" class="radio-type-select" data-id="{{ $i }}" name="type_{{ $i }}" value="{{ TYPE_GRADUAL }}" {{ $schedule->slots[$i]->type == TYPE_GRADUAL ? 'checked' : ''}}>
                                                <span><i class="fas fa-chart-line"></i> GRADUAL</span>
                                            </label>
                                        </div>
                                        <div class="col"></div>
                                        <div class="col"></div>
                    ...
                    <div class="row mt-3">
                        <div class="col-md-12">
                            ...
                            <div class="form-group">
                                <h5>Enable</h5>
                                <label for="moonlight_yes" class="mr-2"><input type="radio" name="moonlight_enabled" id="moonlight_yes" value="1" @if($schedule->moonlight_enabled) checked @endif> Yes</label>
                                <label for="moonlight_no"><input type="radio" name="moonlight_enabled" id="moonlight_no" value="0" @if(!$schedule->moonlight_enabled) checked @endif> No</label>
                            </div>
                        </div>
                    </div>
                    <div class="row mt-3">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <h5>Enable</h5>
                                <label for="geolocation_yes"><input type="radio" name="geo_location" id="geolocation_yes" value="1" {{ $schedule->geo_location ? 'checked' : '' }}> Yes</label>
                                <label for="geolocation_no"><input type="radio" name="geo_location" id="geolocation_no" value="0" {{ !$schedule->geo_location ? 'checked' : '' }}> No</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="location">Location</label>
                                <select name="geo_location_id" id="location" class="form-control" {{ !$schedule->geo_location ? 'disabled' : '' }}>
                                    <option value="">Select Location</option>
                                    @foreach ($locations as $location)
                                    <option value="{{ $location->id }}" {{ $schedule->geo_location_id == $location->id ? "selected" : ''}}>{{ $location->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="water_type" id="water_type" class="form-control" required>
                                    ...
                                    <option value="{{ WATER_MARINE }}" @if($schedule->water_type == WATER_MARINE) selected @endif>{{ WATER_MARINE }}</option>
                                    <option value="{{ WATER_FRESH }}" @if($schedule->water_type == WATER_FRESH) selected @endif>{{ WATER_FRESH }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-12">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <canvas id="spectograph" width="100"></canvas>
            </div>
...
@section('script')
    ...
    <script>
        // $('.radio-type-select').on('change',function(){
        //     let id = $(this).data('id');
        //     let nextId = parseInt(id)+1;
        //     console.log(id,nextId);
        //     if($(this).val() == "{{ TYPE_GRADUAL }}"){
        //         $('#channel_a_end_'+id).prop('readonly',false);
        //         $('#channel_b_end_'+id).prop('readonly',false);
        //         $('#channel_c_end_'+id).prop('readonly',false);
        //         $('#channel_a_start_'+nextId).prop('readonly',true).val($('#channel_a_end_'+id).val());
        //         $('#channel_b_start_'+nextId).prop('readonly',true).val($('#channel_b_end_'+id).val());
        //         $('#channel_c_start_'+nextId).prop('readonly',true).val($('#channel_c_end_'+id).val());
        //     }else {
        //         $('#channel_a_end_'+id).prop('readonly',true).val($('#channel_a_start_'+id).val());
        //         $('#channel_b_end_'+id).prop('readonly',true).val($('#channel_b_start_'+id).val());
        //         $('#channel_c_end_'+id).prop('readonly',true).val($('#channel_c_start_'+id).val());
        //         $('#channel_a_start_'+nextId).prop('readonly',false).val();
        //         $('#channel_b_start_'+nextId).prop('readonly',false).val();
        //         $('#channel_c_start_'+nextId).prop('readonly',false).val();
        //     }
        // })
        ...
        //     let type = $('#row_'+id).find('.radio-type-select:checked').val();
        //     console.log(type);
        //     if(type == "{{ TYPE_STEP }}"){
        //         $('#channel_'+channel+'_end_'+id).val($(this).val());
        //     }
        // });

        // $('.end_value').on('input',function(){
        //     let id= $(this).data('id');
        //     let nextId= parseInt($(this).data('id'))+1;
        //     let channel = $(this).data('channel');
        //     let type = $('#row_'+id).find('.radio-type-select:checked').val();
        //     console.log(type);
        //     if(type == "{{ TYPE_GRADUAL }}"){
        //         $('#channel_'+channel+'_start_'+nextId).val($(this).val());
        //     }
        // });
        // $('#start_time_1').on('change', function () {
        //     $('#start_time_6').val($(this).val());
        // });

        // $('.start-time').on('change', function () {
        //     if ($(this).val() <= $('#start_time_' + $(this).data('id')).val()) {
        //         $(this).val('');
        //         $('#error_' + $(this).data('id')).removeClass('d-none');
        //     } else {
        //         $('#error_' + $(this).data('id')).addClass('d-none');
        //         $('#start_time_' + (parseInt($(this).data('id')) + 1)).val($(this).val());
        //     }
        // });
        $('input[name=geo_location]').on('change', function () {
            if ($(this).val() == '1') {
        ...
        $('.modal-button').on('click', function () {
            ...
            id = $(this).data('id');
            time = $(this).data('start_end');
            var c = $('#channel_a_' + time + '_' + id).val();
            var b = $('#channel_b_' + time + '_' + id).val();
            var a = $('#channel_c_' + time + '_' + id).val();
            if (a > 100 || a < 0) {
                $('.a-error').show();
                return;
            }
            ...
            var channelAResult = [];
            channelA.forEach(element => {
                channelAResult.push(((parseInt(a) / 100) * element) * 0.15);
            });
            var channelBResult = [];
            channelB.forEach(element => {
                channelBResult.push(((parseInt(b) / 100) * element) * 0.45);
            });
            var channelCResult = [];
            channelC.forEach(element => {
                channelCResult.push(((parseInt(c) / 100) * element) * 0.4);
            });
            var sumValues = [];
            var labels = [];
            for (i = 0; i < channelA.length; i++) {
                sumValues.push(parseFloat(channelAResult[i]) + parseFloat(channelBResult[i]) + parseFloat(channelCResult[i]));
                labels.push((380) + i);
            }
            var maxNumber = Math.max.apply(Math, sumValues);
            var Result = [];
            sumValues.forEach(function (element, index) {
                var value = parseFloat(element / maxNumber);
                gradientFill.addColorStop((index / 401), colors[index]);
                Result.push(value);
            });
            console.log(myChart);
            if (myChart) {
